...
var chunk_ZOFGVQ2L_exports = {};
__export(chunk_ZOFGVQ2L_exports, {
...
module.exports = __toCommonJS(chunk_ZOFGVQ2L_exports);
...
async function formatSchema({ schemas }, inputFormattingOptions) {
  if (process.env.FORCE_PANIC_PRISMA_SCHEMA) {
    handleFormatPanic(() => {
      import_chunk_EEYJ3IWX.prismaSchemaWasm.debug_panic();
    });
  }
  ...
  const { formattedMultipleSchemas, lintDiagnostics } = handleFormatPanic(() => {
    ...
    const lintDiagnostics2 = (0, import_chunk_JVM77KBH.lintSchema)({ schemas: formattedMultipleSchemas2 });
    return { formattedMultipleSchemas: formattedMultipleSchemas2, lintDiagnostics: lintDiagnostics2 };
  });
  const lintWarnings = (0, import_chunk_JVM77KBH.getLintWarningsAsText)(lintDiagnostics);
...
function formatWasm(schema, documentFormattingParams) {
  const formattedSchema = import_chunk_EEYJ3IWX.prismaSchemaWasm.format(schema, JSON.stringify(documentFormattingParams));
...
async function processSchemaResult({
  ...
  const configFromPsl = await (0, import_chunk_DYNX76LG.getConfig)({ datamodel: schemaResult.schemas });
...
async function getGenerators(options) {
  ...
  const dmmf = await (0, import_chunk_QRAGQZ4B.getDMMF)({
...
You need to define \`output\` in the generator block in the schema file.`
        ...
        const datamodel = (0, import_chunk_QZK3CVX4.mergeSchemas)({ schemas: schemaContext.schemaFiles });
        const options2 = {
          datamodel,
